...
export class TrackingService {
  ...
  updateTrackOnApp() {
    ...
    if (total == null) {
      ...
      inject(HttpClient).post(SERVER_API + '/students/on_app', {
        'on_total': total
      }).subscribe();
    }
  }
  ...
  updateTrackOnLesson(lessonId: string, trackingType: string) {
    ...
    if (Object.keys(lessonTracking).length === 0 || lessonTracking[trackingType] == null) {
      ...
      inject(HttpClient).post(SERVER_API + '/students/on_lesson', updateTracking).subscribe();
    }
  ...
  resetTrackingOnLesson(lessonId: string, trackingType: string) {
    ...
    inject(HttpClient).post(SERVER_API + '/students/on_lesson', params).subscribe();
  }
